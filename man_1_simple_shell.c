...
.\"Created by Daniel Obigwei and Moses Agbe
...
- Creates a new environment variab;
e or modifies an existing one.
         ...
         - Changes the current directory of the process to DIRECTORY.If no argument is given,
    the command is interpreted as cd $HOME.If the argument - is given, the command is interpreted as cd $OLDPWD.The environment variables PWD and OLDPWD are updated after a change of directory.

                                                                               .B help

                                                                           - Print help message of builtin command

                                                                                 .SH SEE ALSO access(2),
    chdir(2), execve(2), _exit(2), exit(3), fflush(3), fork(2), free(3), isatty(3), getcwd(3), malloc(3), open(2), read(2), sh(1), signal(2), stat(2), wait(2), write(2) Sodash\fR takes basic functionality of the \fBsh\fR shell.This man page is based on the man page bash(1)

                                                                                                                                                                    .SH AUTHORS Ogwalu Chidozie and Mubaraq Abuhasan